   ...
   renderTableBody() {
      return this.state.data.map((data, index) => (
        <React.Fragment key={index}>
          <tr key={index}>
            {Object.keys(data).map((value, columnIndex) => (
              <React.Fragment key={columnIndex}>
                <td
                  key={columnIndex}
                  style={{
                    color:
                      value === "Status" && data[value] === "Paid"
                        ? "green"
                        : value === "Status"
                        ? "red"
                        : value === "Date"
                  ...
                  className={value==="Invoice"||value==="Amount"? "textWhite" : ""}
                >
                  ...
                  {value === "Invoice" ? (
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: "5px"}}>
                      {/* Assuming the image URL is stored in data.imageUrl */}
                      <img src={data.image} style={{marginRight:10}} alt="Profile" width="26" height="26" />
                      <span className='name'>{data[value]}</span>
                    </div>
                  ) : (
                    data[value]
                  )}
